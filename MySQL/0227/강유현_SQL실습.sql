...
select 학번, 이름, substr(이름, 1, 1) as 성
from 학생;
...
from 수강신청, 학생, 학과
where 수강신청.학번 = 학생.학번 && 학생.학과 = 학과.학과번호 && not(학과.학과명 = "컴퓨터정보학과");
...
select 학번, 이름, 학과, if(학과.학과명="컴퓨터정보학과", 학과.학과명, "타과") as 비고
from 학생, 학과
where 학생.학과 = 학과.학과번호;
...
select avg(학점) as 평균학점, sum(시수) as 총학점
from 과목;
...
select 담당교수, count(담당교수) as 과목수, sum(학점) as 학점수
from 과목
group by 담당교수;
...
select 과목번호, count(*) as "수강자 수", avg(평점) as 평균평점
from 수강신청내역
where 평점 != -1
group by 과목번호;
...
select 수강신청번호, count(*) as "수강 과목 수", avg(평점) as 평균평점
from 수강신청내역
group by 수강신청번호
having count(*) >=3
order by 평균평점 desc;
...
from 수강신청내역, 과목, 수강신청
where 수강신청내역.과목번호=과목.과목번호 && 수강신청내역.수강신청번호=수강신청.수강신청번호 && 학번 = 1801001;
-- 67 66과 중복
-- 68 학번이 '1801001'인 학생이 수강 신청한 과목 수와 평점 합계를 출력하는 SQL 문을 작성하시오.
select count(*) as 과목수, sum(평점) as 평점합계
from 수강신청 join 수강신청내역 on 수강신청.수강신청번호 = 수강신청내역.수강신청번호
where 학번 = 1801001
group by 수강신청.학번;
-- 69 수강신청 별로 신청 학생의 학번, 이름, 평균 평점(평점합계/과목수)를 출력하는 SQL 문을 작성하시오. (단, 학점이 부여된 수강신청만 출력한다)
select 수강신청.수강신청번호, 학생.학번, 학생.이름, avg(평점) as 평균평점
from 수강신청내역, 수강신청, 학생
where 수강신청내역.수강신청번호 = 수강신청.수강신청번호 && 수강신청.학번 = 학생.학번
...
select 학생.학번, 학생.이름, count(*) as 과목수, avg(평점) as 평균평점
from 학생, 수강신청, 수강신청내역
where 학생.학번 = 수강신청.학번 && 수강신청.수강신청번호 = 수강신청내역.수강신청번호
group by 수강신청.학번
having 평균평점 > 0;
-- 76 과목별로 수강한 인원, 부여된 평점의 평균을 출력하시오. (단, 1 학기 과목에 한한다)
select 과목.과목번호, 과목.과목명, count(*) as 수강인원, avg(평점) as 평균평점
from 과목, 수강신청내역
where 과목.과목번호 = 수강신청내역.과목번호 && 학기 = 1
