...
where 수강신청.날짜 not like '2018-03%';
-- 5.이름이 ‘김민준’인 학생이 2018학년도 1학기에 수강 신청한 과목명을 검색하시오.
select 과목.설명
from 학생, 수강신청, 수강신청내역, 과목
where 학생.학번 = 수강신청.학번 && 수강신청.수강신청번호 = 수강신청내역.수강신청번호 && 수강신청내역.과목번호 = 과목.과목번호 && 학생.이름 = '김민준' && 수강신청.날짜 like '2018-03%';

select 과목.설명
from 학생 join 수강신청 join 수강신청내역 join 과목 on 학생.학번 = 수강신청.학번 && 수강신청.수강신청번호 = 수강신청내역.수강신청번호 && 수강신청내역.과목번호 = 과목.과목번호
where 학생.이름 = '김민준' && 수강신청.날짜 like '2018-03%';
-- 6.<학생> 테이블의 ‘이름’ 필드를 오름차순으로 정렬하여 출력하는 SQL문을 작성하시오.
select 이름
from 학생
order by 이름;
-- 7.<학생> 테이블의 ‘시도’ 와 ‘이름’ 필드의 데이터를 가져와서 ‘시도’, ‘이름’ 순서로 오름차순으로 정렬하는 SQL문을 작성하시오./ order by 1, 2 는 select 시도(1),이름(2)을 의미함.
select 시도, 이름
from 학생
order by 1, 2;
...
select 학번, 날짜
from 수강신청
where 날짜 between 20180101 and 20180731;
...
select * 
from 수강신청 
where 학번 in('1601001', '1601002') && 연도 = '2016';
...
from 수강신청내역
where 과목번호 in('K20045', 'K20056') && 평점 = 3;
